...
public class CharacterModelController : MonoBehaviour
{
    ...
    //For bringing char back to center
    ...
    protected readonly float k_SlidingColliderScale = 0.5f;
    protected readonly float k_NotSlidingColliderScale = 2f;


    public void ColliderSlide(bool m_Sliding)
    {
        //Move and shrink collider on slide
        if (m_Sliding)
        {
            charCollider.radius = k_SlidingColliderScale * charCollider.radius;
            charCollider.height = k_SlidingColliderScale * charCollider.height;
            charCollider.center = charCollider.center - new Vector3(0.0f, 0.58f, 0.0f);
        }
        else
        {
            charCollider.radius = k_NotSlidingColliderScale * charCollider.radius;
            charCollider.height = k_NotSlidingColliderScale * charCollider.height;
            charCollider.center = charCollider.center + new Vector3(0.0f, 0.58f, 0.0f);
        }
    }
    ...
    {
        if(collision.transform.CompareTag("Level") && m_Jumping)
        {
            character.animator.SetBool(s_JumpingHash, false);
            m_Jumping = false;
        }
